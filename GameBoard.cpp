...
int main() {
	string adventage[24][24] = { {"다같이 마시기", "네. 전부 마시면 됩니다 ^^" },{"황금열쇠", "우리 조가 걸렸을 때 방어할 수 있는 열쇠에요!"},{"으리~ 게임","걸린 조에서 총 3잔을 마시면 됩니다."},{"물 술 물 술","아까 정하신 순서있죠? 순서대로 물드시고 술드시고 물드시고 술드시면 됩니다~ (또 걸리면 술 물 술 물!)"}
...
,{"갓금열쇠","네. 황금 열쇠입니다 :D"},{"가장 먼저 카톡보내기","걸린 조와 상대 조중 더 빨리 진행자에게 카톡으로 \"두근두근배창호\"를 보낸 팀이 승리! 걸린 조가 빠르면 살고, 상대 조가 빠르면 걸린 조가 마십니다^^"},{"처음으로...(숙연)","Ah.. 처음으로 가시겠습니다..^^"},{"START","시작!"} };
...
y = st_y;
gotoxy(x, y);
cout << "게임 진행 순서를 입력해주세요 (1조 먼저, 3조 나중이면 1 3 입력) : ";
cin >> a >> b;
...
bool mooindo[2]; //무인도에 갔는지 여부 표시.
bool soolternull[2];//술터널에 갔는지 여부 표시.
int ternull_space[2];//술터널의 위치 표시.
for (int i = 0; i < team; i++)
{
	mooindo[i] = false;
	soolternull[i] = false;
...
while (true) {
	...
	for (int i = 0; i < team; i++) {
		...
		if (place[i] < 48)
		{
			if(mooindo[i])
			{
				...
				cout << team_first[i] << "조 현재" << "위치 " << place[i] << "입니다.";
				gotoxy(x, ++y);
				...
				if (place[i] >= 8)
				{
					cout << "축하합니다!" << team_first[i] << "조 완주 성공입니다!";
					fin_team++;
					gotoxy(x, ++y);
					cout << "아무 키나 눌러주세요!";
					int key_v3 = _getch();
	...
	for (int i = 0; i < team; i++) {
		gotoxy(x, ++y);
		cout << "현재 " << team_first[i] << "팀 :" << place[i];
	...
	cout << "-----------------------------";
	gotoxy(x, ++y);
	cout << "현재 " << team_first[max_t] << "팀이 위치 " << place[max_t] << "(으)로 선두입니다!";
	Sleep(2000);
	y += 2;
	gotoxy(x, y);
